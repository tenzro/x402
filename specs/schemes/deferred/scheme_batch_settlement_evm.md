...
- **Refund & withdrawal**: **Cooperative refund** — `refund(config, amount)` or `refundWithSignature(config, amount, nonce, sig)` returns up to **`balance - totalClaimed`** to the payer (partial or full). **Timed withdrawal** — `initiateWithdraw(config, amount)` (payer-only) → wait **`withdrawDelay`** → **`finalizeWithdraw(config)`** (caller must be **`payer`** or **`payerAuthorizer`**). Any successful refund clears a pending timed withdrawal for that channel (onchain).
