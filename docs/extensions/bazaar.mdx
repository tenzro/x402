...
<Tabs>
  <Tab title="TypeScript">
    ...
    // Some facilitators return top-level limit/cursor when pagination is used
    if (results.cursor) {
      console.log("Next page cursor:", results.cursor);
    }
  ...
  <Tab title="Go">
    ...
    if results.Cursor != nil {
        fmt.Println("Next cursor:", results.Cursor)
    }
    ```
  ...
  <Tab title="Python">
    ...
    if results.cursor is not None:
        print(f"Next cursor: {results.cursor}")
    ```
