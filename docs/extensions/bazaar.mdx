  ...
  <Tab title="Python">
    ...
    affordable_services = [
        ...
        if any(
            ...
            and int(payment_req.get("maxAmountRequired", 0)) < max_price
...
You can also search for services using a natural-language query. Note that whether results are paginated depends on the facilitator — check `search.paginationSupported` in the response:
...
<Tabs>
  <Tab title="TypeScript">
    ...
    // Check if the server supports stable pagination
    if (results.search.paginationSupported) {
      console.log("Next page cursor:", results.search.cursor);
    }

    console.log(`Found ${results.items.length} services`);
    ```
  ...
  <Tab title="Go">
    ...
    if results.Search.PaginationSupported {
        fmt.Println("Next cursor:", results.Search.Cursor)
    }
    ```
  ...
  <Tab title="Python">
    ...
    print(f"Found {len(results.items)} services")
    if results.search.pagination_supported:
        print(f"Next cursor: {results.search.cursor}")
    ```
